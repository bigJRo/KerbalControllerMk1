/********************************************************************************************************************************
  Time Control Display Module for Kerbal Controller
...
****************************************************************************************/
constexpr uint8_t panel_addr = 0x25;
...
/***************************************************************************************
  Simplified Button Handler for Time Display Module
  - No long press support
  - Each button sets a specific response_value
  - Returns a bitfield of which buttons were triggered
****************************************************************************************/
uint8_t checkButtons() {
  uint8_t triggered = 0;
  ...
  for (uint8_t i = 0; i < NUM_BUTTONS; i++) {
    ...
    if ((millis() - lastDebounceTime[i]) > debounceDelay) {
      ...
      if (!reading && buttonState[i]) {
        ...
        if (!pressHandled[i]) {
          switch (i) {
            case 0: response_value = 1; break;
            case 1: response_value = 60; break;
            case 2: response_value = 1440; break;
          }
          triggered |= (1 << i);
          pressHandled[i] = true;
        }
