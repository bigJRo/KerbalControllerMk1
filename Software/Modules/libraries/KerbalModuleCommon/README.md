...
For modules using **NEO_GRB 3-byte** pixels (SK6812MINI-EA on KC-01-1882), pass colour values directly to `setPixelColor(i, r, g, b)` — the NeoPixel library handles the GRB wire reordering internally. `toGRBW()` is used by modules that need a `GRBWColor` struct for the `ButtonConfig` colour arrays.

---

## System Colour Palette

All colours hardware-validated on SK6812MINI-EA (KC-01-1882 v2.0, NEO_GRB mode).
...
- Entire colour palette hardware-validated on SK6812MINI-EA (KC-01-1882 v2.0, NEO_GRB 3-byte mode)
