...
**Kerbal Controller Mk1 — Resource Display Panel Sketch** · v3.5.0
...
- **Reserve bug.** A touch **held** on the tape for `BUG_HOLD_MS` sets a cyan index at the level first touched (cyan is this project's colour for pilot-entered values), its percent in small cyan figures beside it, and a cyan mark on the band. When the level crosses it, below for a consumable and above for a waste product, the meter's frame and counter take the bug's cyan and the alert strip shows `LF BUG` in cyan; a bug never raises an alarm, and the fixed limit bands outrank it. A touch that lands within `BUG_GRAB_TOL` of an existing bug grabs it: held still it clears the bug, moved more than `BUG_DRAG_MIN_PX` it drags the bug with the finger until release. Bugs are saved with the vessel's slot memory.
...
**Touch.** A tap anywhere on a meter, or on its alert-strip message, opens the Detail screen on that resource. A touch held still on its tape for a second and a half sets a reserve bug there, or clears the bug it landed on; a touch on a bug that moves drags it. The release after a matured hold or a drag does nothing.
