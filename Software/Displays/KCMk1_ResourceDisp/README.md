...
- **Reserve bug.** A touch **held** on the tape for `BUG_HOLD_MS` sets a cyan index at the level first touched (cyan is this project's colour for pilot-entered values), with a cyan mark on the band. When the level crosses it, below for a consumable and above for a waste product, the meter's frame and counter take the bug's cyan and the alert strip shows `LF BUG` in cyan; a bug never raises an alarm, and the fixed limit bands outrank it. A hold within `BUG_CLEAR_TOL` of an existing bug clears it. Bugs are saved with the vessel's slot memory.
