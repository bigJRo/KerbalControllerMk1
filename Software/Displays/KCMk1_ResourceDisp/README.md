...
| Version | Notes |
|---------|-------|
| **3.14.1** | **History time axis in whole minutes.** The tick labels and the `LAST` caption borrowed the counter's time formatter, so at 1x they read `-8:00` and `LAST 9:55`; the ticks are minutes apart and the seconds said nothing. They now read `-10m`, `-8m` ... `-2m` and `LAST 10m`, rounded to the minute, and keep the hours and days forms under warp. |
