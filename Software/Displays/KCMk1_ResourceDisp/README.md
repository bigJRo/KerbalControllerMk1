...
**Kerbal Controller Mk1 — Resource Display Panel Sketch** · v3.14.1
...
The Resource Display is a 1024×600 touchscreen panel that presents real-time KSP resource telemetry sourced from KerbalSimpit. It runs on a Teensy 4.1 and receives telemetry over USB serial from a running KSP instance. The panel tracks up to 16 resource slots simultaneously and supports per-vessel configuration memory for up to 20 vessels, kept in EEPROM across power cycles.
...
| KerbalDisplayAudio | ≥ 1.3.0 | Direct sketch dependency — audio output not used on this panel |
...
| `VESSEL_CACHE_SIZE` | `KCMk1_ResourceDisp.h` | `20` | Maximum number of per-vessel slot configurations remembered, in recency order; the set is mirrored to EEPROM (`Persist.ino`) so it survives a power cycle. |
...
**Time-remaining tiers.** Besides the percent bands, EC, Oxygen, Water and Food alert on the time they have left at the current rate, the same warp-corrected estimate the TTE counter shows: caution below `TIME_WARN_S_*`, alarm below `TIME_ALARM_S_*`, with `TIME_HYST_FRAC` to leave. The O2, water and food times match the Annunciator's TAC-LS thresholds. A state raised this way reads `TIME` in the alert strip. The ten-second rate window cannot see a drain slower than about 5.5 hours to empty; the five-minute `TTE_LONG_WINDOW_MS` window takes over there and reaches about a week, which is what lets the water and food tiers fire at 1x. The Annunciator raises the same tiers from consumption rates, so the two panels normally agree.
...
**Detail** — numerical readout for a single resource. Left panel: selector column with one button per active slot, dimmed for a resource the vessel does not carry. Right panel: resource name header, followed by data rows: **AVAIL** and **TOTAL** (units, with thousands separators and decimals that thin out as the number grows: two under a thousand, one under ten thousand, none above), **REM** (percent), **RATE** (signed, per game second, or per minute or hour when the per-second figure would round to nothing) and **TTE** (time to empty; **TTF**, to full, for a waste resource). The labels are short so the value gets the width. Resources with stage data (LF, LOx, SF, Xenon, Ablator) show the five rows in both CRAFT and STAGE sections; the rest show CRAFT only. Rate and Time are the same estimate the Main screen's trend arrows and TTE counters use (`Sampling.ino`). A **history trace** in a column right of the rows (when `DETAIL_HISTORY`) shows the resource's level over the last ten minutes on a fixed 0 to 100 percent axis, newest at the right, in the resource colour, with the caution and alarm fractions as faint lines and the reserve bug in cyan; it is captioned with the game time it spans, which under warp is hours, on a grid with a mark every 20 percent up the side and every two real minutes along the bottom, the time marks labelled in game time back from now (`-10m` ... `-2m`, `NOW` at the right; hours under warp), exact across warp changes since each sample records the warp in force. The history is kept per resource type in `Sampling.ino` whatever screen is up, so opening Detail shows what was already accumulating; it resets on a vessel switch or scene entry. A **bug bar** along the bottom shows the resource's reserve bug in cyan with `-10` `-1` `+1` `+10` keys to set it to a precise percent and `CLR` to remove it; the first step on a resource without a bug starts one at the caution fraction.
...
| Version | Notes |
|---------|-------|
| **3.14.1** | **History time axis in whole minutes.** The tick labels borrowed the counter's time formatter, so at 1x they read `-8:00`; the ticks are minutes apart and the seconds said nothing. They now read `-10m`, `-8m` ... `-2m`, rounded to the minute, and keep the hours and days forms under warp. The `LAST` caption keeps the `m:ss` form: it grows a sample at a time while the buffer fills, and the seconds are what show that. |
| **3.14.0** | **Bug sweep, optimisation and cleanup pass.** *Fixed:* the inbound I2C command was applied straight from the buffer the receive interrupt writes, so a second command landing mid-read could be torn; it is now snapshotted under `noInterrupts()` first, as on the Annunciator. *Optimised:* the Detail rows formatted every value into a `String` on every pass, ten heap allocations per frame for a screen that mostly shows the same figures; the values now go through fixed buffers with a `strcmp` against the last drawn text, and the thousands separator is built in place, so the pass is heap-free. *Cleaned:* the unused `histGameAgo()` (superseded by the fixed-tick `histGameAgoSamples()` in 3.12.3), the unused style argument to `pitchX()`, a literal `0x08` where KerbalSimpit's `FLIGHT_IS_ATMO_TW` was meant, the `i < 16` in the Detail absent-slot signature (`MAX_SLOTS`), a duplicated `fmtRate` comment, and comments that predated their code: the slot struct "0.0–1.0 floats for demo", "slotCount (4-16)", the vessel memory "persists for the session", a `loop()` note naming meter statics gone since 3.3.0, the time-tier note saying the water and food tiers cannot fire (the long window in 3.11.0 is what lets them), and a Persist header wrapped mid-sentence. *README:* the overview and `VESSEL_CACHE_SIZE` row said the memory was per session; the gestures table had a paragraph glued into its last cell; the counter row description lacked the days forms; the 3.0.1 history row sat out of order; the KerbalDisplayAudio row cited 1.1.0 against a 1.3.0 library. |
