...
`fillArc(tft, cx, cy, rIn, rOut, a0Deg, a1Deg, color)` — annular sector (ring segment) filled pixel-exact between two angles: every pixel at a distance in `[rIn, rOut)` from the centre whose bearing lies within the sweep. Angles are degrees clockwise from 3 o'clock as the screen sees it (y down), `a1Deg > a0Deg`; a sweep of 360° or more fills the whole ring and `rIn = 0` gives a pie slice. Scan-converted into horizontal runs, so the edges and the radial ends land on exact pixels at any radius or stroke width. The RA8876 driver has no arc primitive (its hardware curve command draws whole quadrants only), so this is the arc for limit bands, gauge tracks and dial segments; a chain of dots or thick-line chords beads at small widths.
...
**Roboto Black carries U+00B7 MIDDLE DOT (`0xB7`).** It did not until 3.6.0: the family's second index range ran 128–181 and `0xB7` is 183, so the glyph was simply absent at every size. That matters more than a missing character usually does, because a glyph outside the index range **measures zero and draws nothing** — `getFontStringWidth()` skips it and `kcmDrawString()` skips it — so a separator written as `"\xB7"` renders as an unexplained gap instead of failing. `src/fonts_ili/add_middot.py` adds it, and does not draw it by hand: it lifts each size's own period (`0x2E`) ink and centres it on the cap band measured from that size's capital `H`, so every size gets its own dot shape, weight and side bearings. `0xB6` falls inside the widened range and is given an explicit zero-size glyph, which renders and advances exactly as it did when out of range. The tool decodes every pre-existing glyph before and after and asserts them identical.
...
| **3.7.1** | **Glyph data moved out of DTCM into flash — the build had been one small feature away from failing.** On Teensy 4 a plain `const` array lands in `.data`, and `.data` lives in the half of the 512 KB FlexRAM that ITCM is carved out of. Seventeen fonts is ~189 KB of glyph data, and a build measured `.data` at **195,264 bytes against a DTCM allowance of 196,608** — nothing left for the stack. Because the ITCM/DTCM split is computed *after* linking, the linker never errors; `teensy_size` fails instead, with `exit status 0xffffffff` and no diagnostic. A `KCM_FONT_FLASH` section attribute on each font's `_data[]` and `_index[]` moves all 34 arrays to `.progmem`. Flash is memory-mapped on this part, so the renderer dereferences the same pointers with no `pgm_read_*` dance and no code change; the cost is that glyph fetches go through the FlexSPI cache rather than DTCM. The macro is a no-op off-target so the host harnesses still build the same files. RAM1 went from a **15,840-byte deficit to 144,928 bytes free**. Note the fix worked by freeing DTCM, not by shedding a bank: ITCM is still ten 32 KB banks either way. |
| **3.7.0** | **`kcmRateUpdate`/`kcmRateReset` — body angular rates for the Shuttle-style rate pointers.** Derived from the relative rotation of two successive `kspBodyAxes` frames (`dR = R₁ᵀR₂`, rotation vector from the skew-symmetric part) rather than by differentiating heading/pitch/roll. The distinction is the whole point: the Euler derivative blows up near ±90° pitch, which is exactly where a launch spends its first minute, but that is a defect of the parameterisation and not of the motion — differentiating the *rotation* stays well conditioned everywhere. Signs follow the aviation convention (roll +, pitch +, yaw negated). Note that `KspBodyAxes` stores `(fwd, right, up)` with `up = right × fwd`, which is left-handed; the right-handed triad used internally is `(fwd, up, right)`. Also promotes `drawRoundRectOutline` in from `KCMk1_InfoDisp`, as its own note there anticipated, now that the reference chips are its second and third callers. |
| **3.6.0** | **U+00B7 MIDDLE DOT added to all ten Roboto Black sizes.** The family's second index range ended at 181 and `0xB7` is 183, so the glyph was absent everywhere — and an absent glyph measures zero and draws nothing rather than erroring, so a separator written as `"\xB7"` silently rendered as a gap. `fonts_ili/add_middot.py` extends `index2_last` 181 → 183 and builds the dot from each size's own period ink, centred on that size's cap band; `0xB6` gets an explicit zero-size glyph inside the widened range. Every pre-existing glyph is decoded before and after and asserted identical — 149 per font, all ten fonts. Costs 10–146 bytes of flash per size. Additive only: no existing glyph, metric or API changes. |
