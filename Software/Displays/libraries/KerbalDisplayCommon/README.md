...
**Kerbal Controller Mk1 — Shared Display Library** · v3.4.0
...
| Version | Notes |
|---------|-------|
| **3.4.0** | New **`drawTargetProgradeMarker`** — prograde's three spokes and centre dot drawn with the target marker's four-arc gapped ring. The plain prograde glyph meant *orbital/surface* velocity on the PFD ball and *target-relative* velocity on the TGT/DOCK reticles: identical symbol, identical neon green, two different reference frames, nothing on screen to tell them apart. Borrowing the target ring keeps the KSP reading (prograde is prograde) and avoids spending a colour, which would have collided with the yellow/red warning palette. The gap is 26° rather than the target marker's 15°: that marker has no spokes, so a 30° gap reads as a gap, but here the spoke plus the arcs' round end-caps swallow most of it and the ring comes out looking solid. `reticleUpdateDots` now draws the velocity marker with it; `drawProgradeMarker` is unchanged and still serves the PFD. |
