...
static void drawScreen_MNVR(RA8875 &tft) {
  ...
  // Row 4 — Maneuver ΔV required (reference value, always green)
  mnvrVal(4, "\xCE\x94V.Mnvr:", fmtMs(state.mnvrDeltaV), TFT_DARK_GREEN, TFT_BLACK);

  // Row 5 — Burn duration (renamed T.Burn for consistency, always green)
  mnvrVal(5, "T.Burn:", formatTime(state.mnvrDuration), TFT_DARK_GREEN, TFT_BLACK);

  // Row 6 — Total ΔV remaining:
  //   white-on-red if < maneuver ΔV (can't complete burn)
  //   yellow if within 10% of maneuver ΔV (marginal)
  ...
  // Split: M.Hdg (left, cache[5][7]) | M.Pit (right, cache[5][8])
  // printState bug fix: each cell uses its own slot
  {
