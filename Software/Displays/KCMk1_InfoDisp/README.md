...
**Kerbal Controller Mk1 — Information Display Panel Sketch** · v1.4.1
...
| 3 | Recoverable vessel **and not pre-launch** | VEHICLE INFO |
| 4 | Everything else | SPACECRAFT (PFD) |

Two things about rule 3. It sits *below* the vessel-type rules because KSP reports anything on the home body as recoverable: a rover being driven on Kerbin, or a plane rolling out after landing, is recoverable, and above them this rule would replace the instrument for the vehicle actually being operated with a static summary. And the pre-launch exclusion is load-bearing for the same reason — a vessel on the pad is recoverable too, and the PFD is what the pilot wants there.
...
**AUTO / MAN chip.** The title bar states, in words, whether the screen was chosen by the ladder or is being held by hand — MAN in cyan, the panel's existing colour for a pilot-selected value. It covers the LAUNCH screen's own ASC/CIRC phase override too, which replaced that screen's red override dot.
