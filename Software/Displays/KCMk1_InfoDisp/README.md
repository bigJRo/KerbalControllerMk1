...
**Kerbal Controller Mk1 — Information Display Panel Sketch** · v1.4.0
...
|---|---|---|
| 1 | Recoverable vessel **and not pre-launch** | VEHICLE INFO |
| 2 | Rover | ROVER |
| 3 | Plane in the atmosphere | AIRCRAFT |
| 4 | Everything else | SPACECRAFT (PFD) |

The pre-launch exclusion on rule 1 is load-bearing: KSP reports a vessel on the pad as recoverable, so without it VEHICLE INFO would come up on the pad — exactly where the PFD is wanted. The old combined ladder never hit this because its pre-launch rule sat above its recoverable rule and shielded it; with the ladders split, the shield has to be explicit.
